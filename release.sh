...
if $CUT_ZIP; then
    ...
    rm -f "$ZIPNAME"
    zip -X -r "$ZIPNAME" \
        README.md CHANGELOG.md MANUAL.md TESTING.md TS_PROGRESS.md VERSION LICENSE \
        Makefile Dockerfile docker-compose.yml run_tests.sh syncver.sh release.sh \
        go.mod go.sum \
        cmd/ pkg/ docs/ scripts/ \
