...
//	    per-tenant sequences, FTS rebuild, graph_edges table.
//
...
func migrateV1ToV2(db *sql.DB, dryRun, verbose bool) error {
	...
	if dryRun {
		...
		log.Println("  - Create graph_edges table (if missing)")
	...
	// 4. Ensure graph_edges table exists
	if verbose {
		log.Println("  Ensuring graph_edges table exists...")
	}
	_, err = tx.Exec(`CREATE TABLE IF NOT EXISTS graph_edges (
