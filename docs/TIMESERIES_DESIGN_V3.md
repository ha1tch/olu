...
### 4.1 Tenant mode restriction

Timeseries storage is available **only in strict tenant mode**
(`TenantMode: "strict"`). In path mode, tenants auto-register on first
access, and the lifecycle of per-tenant Pebble instances cannot be
managed safely without explicit provisioning.

When `TenantMode` is not `"strict"`, the timeseries subsystem does not
initialise. API requests to timeseries endpoints return `403 Forbidden`
with error code `OLU-TS001`.
...
- If `TimeseriesEnabled` is true and `TenantMode` is not `"strict"`, the
  server refuses to start with a clear error message.
...
New fields in the olu configuration:
    ...
    # Feature flag (requires TenantMode = "strict")
...
> **Implementation status:** The `olu-admin` binary described in this
...
data volume and does not block reads or writes.
    ...
    olu-admin ts backup --tenant acme --output /backup/acme-ts-20260302/
...
Restore replaces the tenant's Pebble data directory with a backup:
    ...
    olu-admin ts restore --tenant acme --input /backup/acme-ts-20260302/
...
4. **Purge from live.** Delete the archived key range using bounded
   ...
   immediately:
       ...
       olu-admin ts compact --tenant acme
...
**Thaw to live store:**
    ...
    olu-admin ts thaw --tenant acme --archive 2025-12
...
**Thaw to temporary read-only instance:**
    ...
    olu-admin ts thaw --tenant acme --archive 2025-12 --readonly
...
### 14.4 Capacity planning
    ...
    olu-admin ts growth --tenant acme
