...
```
olu-admin tenant unlock --name acme --force
```
...
1. Operator runs `olu-admin tenant create --name acme --instance hot-A`.
...
build-admin:
    go build -tags admin -o olu-admin ./cmd/olu-admin
```
...
```
olu-admin tenant create   --name acme --instance hot-A
olu-admin tenant disable  --name acme --reason "billing"
olu-admin tenant freeze   --name acme
olu-admin tenant thaw     --name acme --instance hot-B
olu-admin tenant delete   --name acme --confirm
olu-admin tenant list     [--status active] [--instance hot-A]
olu-admin tenant inspect  --name acme
```

**Data lifecycle:**

```
olu-admin archive run     [--tenant acme] [--entity events] [--before 2026-01-01]
olu-admin archive status  --tenant acme
olu-admin archive restore --tenant acme --month 2026-01 --instance hot-A
```

**Fleet operations:**

```
olu-admin instance register  --name hot-C --endpoint https://...
olu-admin instance drain     --name hot-A
olu-admin instance status
olu-admin migrate tenant     --name acme --from hot-A --to hot-B
```

**Reporting:**

```
olu-admin report usage       [--tenant acme]
olu-admin report storage     [--instance hot-A]
olu-admin report audit       [--since 2026-02-01] [--action tenant.*]
```
...
short-lived HTTP server (`olu-admin serve --port 9090 --timeout 30m`) that
...
```
olu-admin query --tenant acme --archive 2026-01 \
...
- Add `olu-admin archive run` scheduled operation.
...
- Tenant migration between instances (`olu-admin migrate tenant`).
