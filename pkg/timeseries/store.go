...
import (
	...
	"github.com/ha1tch/olu/pkg/tdigest"
)
...
func NewPebbleStore(dir string, cfg StoreConfig) (Store, error) {
	...
	opts := &pebble.Options{
		...
		L0StopWritesThreshold: cfg.L0CompactionThreshold * 4,
	}
	switch cfg.Compression {
	case "zstd":
		opts.Levels = []pebble.LevelOptions{{Compression: pebble.ZstdCompression, BlockSize: cfg.BlockSize}}
	case "snappy":
		opts.Levels = []pebble.LevelOptions{{Compression: pebble.SnappyCompression, BlockSize: cfg.BlockSize}}
	default:
		opts.Levels = []pebble.LevelOptions{{Compression: pebble.NoCompression, BlockSize: cfg.BlockSize}}
	}
	...
	if err := s.loadMeta(cfg.Compression); err != nil {
		db.Close()
...
// NewPebbleStoreFactory returns a StoreFactory backed by Pebble.
func NewPebbleStoreFactory() StoreFactory {
	return func(dir string, cfg StoreConfig) (Store, error) {
		return NewPebbleStore(dir, cfg)
	}
}
...
func (s *PebbleStore) Aggregate(ctx context.Context, q AggregateQuery) ([]Bucket, error) {
	...
	endKey := incrementKey(mustEncodeKey(q.Timeline, cfg.Dims, padDims(q.Dims, cfg.Dims, math.MaxUint64), q.To))
...
// mustEncodeKey panics on error; used only in contexts where inputs are
// already validated.
func mustEncodeKey(tid TimelineID, dims uint8, dv []uint64, ts time.Time) []byte {
	k, err := EncodeKey(tid, dims, dv, ts)
	if err != nil {
		panic(err)
	}
	return k
