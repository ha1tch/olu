...
func TestOQLDateTrunc(t *testing.T) {
	...
	t.Run("DATE_TRUNC hour groups correctly", func(t *testing.T) {
		// Shelf's exact hourly analytics query
	...
	t.Run("DATE_TRUNC day groups correctly", func(t *testing.T) {
		// Shelf's exact daily analytics query
		data := env.oqlData("/api/v1/oql/query",
	...
	t.Run("DATE_TRUNC with WHERE range filter", func(t *testing.T) {
		// Shelf's exact query with time range
		data := env.oqlData("/api/v1/oql/query",
...
func TestOQLGroupByIntegration(t *testing.T) {
	...
	t.Run("GROUP BY trigger_type with COUNT (Shelf's exact query)", func(t *testing.T) {
		data := env.oqlData(
...
// ============================================================================
// 4. COMBINED: Shelf's full event analytics workflow
//    Create events -> query with DATE_TRUNC -> verify tenant isolation
// ============================================================================

func TestShelfEventAnalyticsWorkflow(t *testing.T) {
	env := newShelfTestEnv(t)
	defer env.cleanup()
	...
	t.Run("Daily breakdown (Shelf's group_by=day query)", func(t *testing.T) {
	...
	t.Run("Hourly breakdown (Shelf's group_by=hour query)", func(t *testing.T) {
		data := env.oqlData(base+"/oql/query",
	...
	t.Run("By trigger_type (Shelf's group_by=trigger_type query)", func(t *testing.T) {
		data := env.oqlData(base+"/oql/query",
