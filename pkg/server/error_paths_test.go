...
func TestErrorPaths_SaveConflict(t *testing.T) {
	ts := setupTestServer(t)
	defer ts.cleanup()
	...
	// Try to save at same ID — should conflict
	t.Run("save existing id", func(t *testing.T) {
		resp, body := ts.doRequest("POST", "/api/v1/items/save/1",
			map[string]interface{}{"name": "duplicate"})
		if resp.StatusCode != http.StatusConflict {
			t.Errorf("expected 409, got %d: %s", resp.StatusCode, string(body))
		}
	})
