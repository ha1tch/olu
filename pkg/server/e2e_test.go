...
// Ordered by frequency of use in a typical Shelf AMS deployment:
//
//   1. Multi-step Shelf workflows (CRUD → graph → OQL analytics)
...
// shelf_integration_test.go (Shelf-specific OQL/mutation paths), and
// shelf_tier1_test.go (analytics/scalar functions).
//
...
// e2eEntities covers the Shelf schema plus a few extras for graph testing.
...
// =============================================================================
// 1. MULTI-STEP SHELF WORKFLOWS
//    These test realistic sequences that Shelf AMS performs.
// =============================================================================
...
func TestE2E_SaveEndpoint(t *testing.T) {
	...
	t.Run("save with existing ID returns conflict", func(t *testing.T) {
		status, _ := env.doJSON("POST", "/api/v1/assets/save/100", map[string]interface{}{
			"code": "SAVE-002",
			"name": "Duplicate",
		})
		if status != http.StatusConflict {
			t.Errorf("save duplicate expected 409, got %d", status)
		}
	})
...
// TestE2E_GraphCommonNeighbors tests POST /api/v1/graph/commonNeighbors
func TestE2E_GraphCommonNeighbors(t *testing.T) {
	env := newE2EEnv(t)
	defer env.cleanup()
