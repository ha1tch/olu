...
// shelf_integration_test.go
//
// Comprehensive integration tests targeting the olu API surface areas that
// Shelf AMS relies on most heavily. Written to exercise the exact code paths
// Shelf uses, with special focus on areas where olu had weak or no test
...
// shelfTestEnv holds a fully configured test server with OQL support and
// pre-created entity directories matching Shelf's schema.
...
// shelfEntities lists all entity types Shelf uses. These directories must
// exist for the OQL validator to accept queries against them.
var shelfEntities = []string{
	"assets", "events", "asset_types", "fsm_machines", "rules",
	"sensors", "sensor_bindings", "qr_codes", "qr_scans",
	"users", "webhooks",
}

func newShelfTestEnv(t *testing.T) *shelfTestEnv {
	t.Helper()

	tmpDir, err := os.MkdirTemp("", "olu-shelf-integ-*")
	if err != nil {
		t.Fatal(err)
	}

	// Pre-create entity directories so the OQL validator recognises them.
	for _, entity := range shelfEntities {
		dir := filepath.Join(tmpDir, "shelf_schema", entity)
	...
	cfg := &config.Config{
		...
		BaseDir:             tmpDir,
		Schema:              "shelf_schema",
		SchemaDir:           filepath.Join(tmpDir, "shelf_schema"),
		CacheType:           "memory",
		CacheTTL:            300,
		GraphEnabled:        true,
		GraphMode:           "indexed",
	...
	return &shelfTestEnv{ts: ts, tmpDir: tmpDir, t: t}
}

func (e *shelfTestEnv) cleanup() {
	e.ts.Close()
	os.RemoveAll(e.tmpDir)
}

// do makes an HTTP request and returns the response + body bytes.
func (e *shelfTestEnv) do(method, path string, body interface{}) (*http.Response, []byte) {
	e.t.Helper()
	var bodyBytes []byte
	if body != nil {
		var err error
...
func (e *shelfTestEnv) doJSON(method, path string, body interface{}) (int, map[string]interface{}) {
	e.t.Helper()
...
func (e *shelfTestEnv) createEntity(path string, data map[string]interface{}) int {
	e.t.Helper()
...
func (e *shelfTestEnv) oql(path, query string) (int, map[string]interface{}) {
	e.t.Helper()
	return e.doJSON("POST", path, map[string]interface{}{"query": query})
}

// oqlData executes an OQL SELECT and returns the data rows.
func (e *shelfTestEnv) oqlData(path, query string) []interface{} {
	e.t.Helper()
...
func (e *shelfTestEnv) graphStats() (int, int) {
	e.t.Helper()
...
func (e *shelfTestEnv) graphNeighbors(nodeID, direction string) map[string]interface{} {
	e.t.Helper()
...
//    Shelf uses these for pre-delete referential integrity checks.
// ============================================================================
...
func TestOQLCountAggregates(t *testing.T) {
	...
	t.Run("COUNT(*) with WHERE on integer field", func(t *testing.T) {
		// This is Shelf's exact query for checking if an asset_type has assets
...
//    Shelf uses UPDATE via OQL for sensor unbinding.
// ============================================================================
...
func TestOQLUpdateMutations(t *testing.T) {
	...
	// Create sensor bindings (Shelf's exact pattern)
	...
	t.Run("UPDATE SET with WHERE on integer field", func(t *testing.T) {
		// This is Shelf's exact unbind query
...
//    Shelf's most common OQL pattern: WHERE code = 'X', WHERE email = 'X'
// ============================================================================
...
func TestOQLStringEquality(t *testing.T) {
	...
	t.Run("SELECT TOP 1 with string WHERE (QR lookup)", func(t *testing.T) {
		// Shelf's exact QR code lookup query
		data := env.oqlData("/api/v1/oql/query",
	...
	t.Run("SELECT TOP 1 with string WHERE (user login)", func(t *testing.T) {
		// Shelf's exact login lookup query
		data := env.oqlData("/api/v1/oql/query",
	...
	t.Run("SELECT id field only (Shelf QR id lookup)", func(t *testing.T) {
		data := env.oqlData("/api/v1/oql/query",
...
//    Shelf patches assets on every FSM transition. This is the truth layer
...
func TestPatchGraphEdgeConsistency(t *testing.T) {
	...
	t.Run("PATCH non-REF field preserves graph edges", func(t *testing.T) {
		// Shelf's FSM state transition: only patches fsm_state
...
func TestShelfEventWorkflow(t *testing.T) {
	env := newShelfTestEnv(t)
	defer env.cleanup()
	...
	// 5. Simulate FSM transition (Shelf patches only fsm_state)
	status, _ := env.doJSON("PATCH",
	...
	t.Run("Pre-delete check: COUNT(*) prevents orphan deletion", func(t *testing.T) {
		// Shelf checks this before deleting an asset_type
		data := env.oqlData(base+"/oql/query",
...
func TestEdgeCases(t *testing.T) {
	...
	t.Run("OQL query on mixed numeric and string fields", func(t *testing.T) {
		...
		// Shelf's exact event query pattern
		data := env.oqlData("/api/v1/oql/query",
...
// ============================================================================
// 10. MULTIPLE REFs ON SAME ENTITY (Shelf assets have type_ref + sensor_ref + fsm_ref)
// ============================================================================
