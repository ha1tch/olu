...
func TestSQLiteTenantIsolation_RebuildGraph(t *testing.T) {
	...
	// Verify edge counts per tenant via GetNeighbors
	graphA := sA.(GraphNeighbors)
	neighborsA, _ := graphA.GetNeighbors(ctx, "tasks", 1, "out")
	if len(neighborsA) != 1 {
		t.Errorf("tenant A neighbors after rebuild = %d, want 1", len(neighborsA))
	}

	graphB := sB.(GraphNeighbors)
	neighborsB, _ := graphB.GetNeighbors(ctx, "tasks", 1, "out")
	if len(neighborsB) != 1 {
		t.Errorf("tenant B neighbors after rebuild = %d, want 1", len(neighborsB))
	}
...
func TestSQLiteTenantIsolation_ZeroAndNonZeroGraphs(t *testing.T) {
	...
	// Tenant 0 uses default graph_edges table
	...
	store0.db.QueryRow("SELECT COUNT(*) FROM graph_edges").Scan(&defaultCount)
	store0.db.QueryRow("SELECT COUNT(*) FROM graph_t0001").Scan(&tenantCount)

	if defaultCount == 0 {
		t.Error("default graph_edges should have edges from tenant 0")
	}
	if tenantCount == 0 {
		t.Error("graph_t0001 should have edges from tenant 1")
	}

	// Neighbors are scoped
	var s0 Store = store0
	var s1 Store = store1
	graph0 := s0.(GraphNeighbors)
	graph1 := s1.(GraphNeighbors)

	n0, _ := graph0.GetNeighbors(ctx, "edges_data", 1, "out")
	n1, _ := graph1.GetNeighbors(ctx, "edges_data", 1, "out")
	...
	if len(n0) == 1 {
		if label, _ := n0[0]["label"].(string); label != "root-0" {
			t.Errorf("tenant 0 neighbor label = %q, want root-0", label)
		}
	}
	if len(n1) == 1 {
		if label, _ := n1[0]["label"].(string); label != "root-1" {
