...
func TestSQLiteTenantIsolation_Save(t *testing.T) {
	...
	err := storeA.Save(ctx, "records", 42, map[string]interface{}{"owner": "A"})
	if err != nil {
		t.Fatalf("storeA.Save: %v", err)
	}
	err = storeB.Save(ctx, "records", 42, map[string]interface{}{"owner": "B"})
	if err != nil {
...
func TestSQLiteTenantIsolation_GraphEdges(t *testing.T) {
	...
	neighborsA, err := storeA.GetNeighbors(ctx, "tasks", 1, "out")
	if err != nil {
		t.Fatalf("storeA.GetNeighbors: %v", err)
	}
	if len(neighborsA) != 1 {
		t.Errorf("tenant A neighbors = %d, want 1", len(neighborsA))
	}
	if len(neighborsA) == 1 {
		if name, _ := neighborsA[0]["name"].(string); name != "Project-A" {
			t.Errorf("tenant A neighbor name = %q, want Project-A", name)
		}
	}

	neighborsB, err := storeB.GetNeighbors(ctx, "tasks", 1, "out")
	if err != nil {
		t.Fatalf("storeB.GetNeighbors: %v", err)
	}
	if len(neighborsB) != 1 {
		t.Errorf("tenant B neighbors = %d, want 1", len(neighborsB))
	}
	if len(neighborsB) == 1 {
		if name, _ := neighborsB[0]["name"].(string); name != "Project-B" {
...
func TestGraphEdgesTable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("tenant_%04X", tt.tenantID), func(t *testing.T) {
			got := graphEdgesTable(tt.tenantID)
			if got != tt.want {
				t.Errorf("graphEdgesTable(%d) = %q, want %q", tt.tenantID, got, tt.want)
			}
		})
	}
}
