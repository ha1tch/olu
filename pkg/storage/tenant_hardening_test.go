...
//
// Tenant 0 uses graph_edges (default table). Deleting in tenant 0
// must not touch per-tenant graph tables, and vice versa.
// ---------------------------------------------------------------------------
...
func TestSQLiteTenantIsolation_GraphEdgeCleanupTenantZeroVsNonZero(t *testing.T) {
	...
	store0.db.QueryRow("SELECT COUNT(*) FROM graph_edges").Scan(&count0)
	store0.db.QueryRow("SELECT COUNT(*) FROM graph_t0001").Scan(&count1)
	if count0 < 1 {
		t.Fatalf("baseline: graph_edges (tenant 0) = %d, want >= 1", count0)
	}
	if count1 < 1 {
	...
	// graph_edges should have fewer edges
	var count0After int
	store0.db.QueryRow("SELECT COUNT(*) FROM graph_edges WHERE source_entity='children' AND source_id=1").Scan(&count0After)
	if count0After != 0 {
		t.Errorf("after delete: graph_edges still has children:1 edges = %d", count0After)
	}
