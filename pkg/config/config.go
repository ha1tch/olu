...
type Config struct {
	...
	// Timeseries storage (Pebble-backed, requires TenantMode = "strict" and StorageType = "sqlite")
...
func LoadFromEnv(cfg *Config) {
	...
	if val := os.Getenv("OLU_JWT_SECRET"); val != "" {
		cfg.JWTSecret = val
	}
...
func (c *Config) Validate() (errs []string, warnings []string) {
	...
	check("GraphMode", c.GraphMode, []string{"indexed", "disabled"})
	...
	check("AuthType", c.AuthType, []string{"none", "jwt", "apikey"})
	...
	// Multi-tenant graph enforcement: the graph layer is a single shared
	// in-memory structure without per-tenant isolation. In strict mode,
	// force it off to prevent unscoped graph queries across tenants.
	if c.TenantMode == "strict" && c.GraphEnabled {
		c.GraphEnabled = false
		c.GraphMode = "disabled"
		warnings = append(warnings,
			"GraphEnabled forced to false: graph is not tenant-isolated and cannot be used in strict mode")
	}
	...
	if c.TimeseriesEnabled {
		if c.TenantMode != "strict" {
			errs = append(errs, "TimeseriesEnabled requires TenantMode \"strict\"")
		}
		if c.StorageType != "sqlite" {
